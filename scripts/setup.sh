...
# setup.sh — One-command setup for button-hub on a fresh Debian/Ubuntu machine.
# Run this as a regular user with passwordless sudo or be ready to enter
# your password when prompted.
#
# What it does:
#   - Installs system packages (mosquitto, git, curl, build-essential)
...
if ! command -v sudo &>/dev/null; then
    log_error "sudo is required. Install it first: apt-get install sudo"
    exit 1
fi
...
if ! command -v apt-get &>/dev/null; then
    log_error "This script requires apt-get (Debian/Ubuntu). Manual setup needed for other distros."
    exit 1
fi
...
sudo apt-get install -y -qq mosquitto mosquitto-clients git curl build-essential pkg-config libssl-dev && log_info "Packages installed" || {
    log_error "Failed to install packages. Check your internet connection and apt sources."
    exit 1
}
